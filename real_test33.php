...
echo '</h1>';
	...
	if ( (isset($_POST['id'])) && (is_numeric($_POST['id'])) )
	{
		...
		$order = $_SESSION['order'];
	       ...
	       if(isset($_POST['correct_answer']))
		   {
			...
			// Make sure the answer is not saved erailer 
			
			$q = "SELECT answer_id , exam_id , question_id , question_order, user_id , selected_answer, correct_answer FROM answer WHERE ( question_order='$page' AND user_id='$user')";		
		    $r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));
		
		if (mysqli_num_rows($r) > 0) { // there is exsiting answer.
			
			echo '<center><p style="color:red;font-size:17px">تمت اختيار الاجابة مسبقاً<p></center>';
			 $row = mysqli_fetch_array($r, MYSQLI_NUM); // to bring the earlier selected answer 
			 $s_answer = $row[5];
		...
		else 
		{
			...
			$q = "INSERT INTO answer (exam_id, question_id, question_order, user_id, selected_answer, correct_answer) VALUES ('$eid', '$question_number', '$page','$user', '$selected_answer', '$c_answer')";
			$r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));
	...
	else 
	{
			...
			$s_answer = 0; // there is no need to know selected answer in first question
	}
	...
	if ($questions_count !=0)
	{
		...
		if (mysqli_num_rows($r) > 0 )
		{
			...
			echo '<form action="real_test33.php" method="post">';
...
<tr>';
             ...
             echo'<td> <input type="radio" name ="correct_answer" value="1" checked> ' . $row[2] . ' <br>
             ...
             echo'<td> <input type="radio" name ="correct_answer" value="1" > ' . $row[2] . ' <br>
