...
// لمعرفة اذا كانت العضوية مجانية فلا يمكن انشاء اكثر من 100 سؤال
	...
	if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ) { // From teacher.php
		...
		$r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));
			...
			<li ><a href="test_control.php?id=' .  $id . '">نشر الاختبار</a></li>
		</ol>
		<br>';

								echo'<div id="logo">
		            <h3>إضافة أسئلة الاختبار:  ' . $row['exam_name'] . ' </h3>
		            </div>';
    ...
    */
	...
	if ($qn && $an1 && $an2 && $an3 && $an4) { // If everything's OK...
			...
			$q = "INSERT INTO question (exam_id, user_id, question, description, type, answer1, answer2, answer3, answer4, correct_answer) VALUES ('$id', '$u', '$qn', '$d', '1', '$an1', '$an2' ,'$an3', '$an4', '$co' )";
			$r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));
...
<div class="container-fluid">
  ...
  <form action="add_question2.php" method="post" role="form">
